...
export default function QuizCard() {
  ...
  const mintNFT = async () => {
    console.log("minting.....");
    await writeAsync();
    await tableLandMinting();
  };

  const { writeAsync } = useScaffoldContractWrite({
    contractName: "QuizNFT",
    functionName: "mintNTFQuiz",
    // args: [postId, amount],
  ...
  // generateNFT Tableland
  const quizName = "ReactJS Basics";
  const finalScore = 100;
  const quizId = 900;

  const { writeAsync: tableLandMinting } = useScaffoldContractWrite({
    contractName: "TablelandNFT",
    functionName: "addProfile",
    args: [quizName, finalScore, quizId],
    blockConfirmations: 1,
    onBlockConfirmation: txnReceipt => {
      console.log("Transaction blockHash", txnReceipt.blockHash);
    },
        ...
        <div>
          ...
          {score(ans, 1) === TOTAL_NUMBER_QUESTIONS && (
            <div className="bg-white rounded-lg shadow-md px-4 py-6 border border-gray-300 text-center pb-8">
              ...
              <button className="p-3 text-white bg-green-500 hover:bg-green-600 rounded-lg" onClick={mintNFT}>
                Mint NFT
              </button>
            </div>
          )}
